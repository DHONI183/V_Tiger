...
public class Create_Org_Test_3rd_Script {
	...
	public void createNewOrgTest_Excel_3rd_script() throws InterruptedException, IOException {
		...
		// Fetch the data
		String industry_drop = c4.toString();

		// Fetch the Cell
		Cell c5 = r.getCell(5);

		// Fetch the data
		String type_drop = c5.toString();
		...
		dropIndustry.selectByValue(industry_drop);

		// type drop down
		WebElement type = driver.findElement(By.xpath("//select[@name='accounttype']"));
		type.click();
		Select dropType = new Select(type);
		dropType.selectByValue(type_drop);
		...
		// identify industry dropdown
		WebElement industry_d = driver.findElement(By.id("mouseArea_Industry"));
		
		// identify Type dropdown
		WebElement type_d = driver.findElement(By.id("mouseArea_Type"));
		

		if (header.getText().contains(orgName) && verifyphno.getText().contains(phno) && industry_d.getText().contains(industry_drop) && type_d.getText().contains(type_drop) ) {
			System.out.println("create Org with phno Verified OrgName, phno , industry and type test pass");
		...
		driver.findElement(
				By.xpath("//a[text()='Tek Pyramid']/ancestor::tr[@bgcolor=\"white\"]/descendant::a[text()='del']"))
				.click();
